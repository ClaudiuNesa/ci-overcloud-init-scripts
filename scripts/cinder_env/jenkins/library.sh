...
join_cinder(){
    ...
    set -e
    run_wsmancmd_with_retry "powershell -ExecutionPolicy RemoteSigned git clone https://github.com/cloudbase/ci-overcloud-init-scripts.git C:\ "
    run_wsmancmd_with_retry "bash C:\OpenStack\devstack\scripts\gerrit-git-prep.sh --zuul-site $ZUUL_SITE --gerrit-site $ZUUL_SITE --zuul-ref $ZUUL_REF --zuul-change $ZUUL_CHANGE --zuul-project $ZUUL_PROJECT"
