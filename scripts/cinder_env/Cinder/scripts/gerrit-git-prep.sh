...
then
    ...
    then
        git clone file:///opt/git/$ZUUL_PROJECT C:/$ZUUL_PROJECT
    else
        git clone $GIT_ORIGIN/openstack/$ZUUL_PROJECT C:/$ZUUL_PROJECT
    fi
fi

git remote set-url origin $GIT_ORIGIN/openstack/$ZUUL_PROJECT
...
then
    git fetch $ZUUL_SITE/p/openstack/$ZUUL_PROJECT $ZUUL_REF
